...
class ClientServerAPI():
    def __init__(self, task_id):
        ...
        self.ccl_address = os.getenv('FEDOPS_AGGREGATION_HOST', '').strip()
    ...
    def get_port(self):
        ...
        if response.status_code == 200:
            public_host = os.environ.get('FEDOPS_AGGREGATION_HOST', '').strip()
            if not public_host:
                raise ValueError('FEDOPS_AGGREGATION_HOST is required when FEDOPS_AGGREGATION_SERVER is unset')
